...
END
GO
CREATE PROCEDURE dbo.uspProdTrackDetails2 
	@CREDAT DATETIME, 
...
SELECT 
	...
	ht.CREDAT_0
INTO #Trk
FROM ECCPROD2.MFGHEADTRK ht
	LEFT JOIN ECCPROD2.MFGITMTRK itm
		ON ht.MFGTRKNUM_0 = itm.MFGTRKNUM_0
	LEFT JOIN ECCPROD2.MFGMATTRK mat
		ON ht.MFGTRKNUM_0 = mat.MFGTRKNUM_0
	LEFT JOIN ECCPROD2.MFGOPETRK ope
...
SELECT '***' SumByWorkOrderForMissingTrackings, SUM(QTYSTU_0) SumQtyStu, VCRNUMORI_0
FROM ECCPROD2.STOJOU s
WHERE 
	VCRNUMORI_0 IN 
		(SELECT DISTINCT MFGNUM_0 FROM #Trk)
		AND TRSTYP_0 IN(5,6, 35)
...
BEGIN
	SELECT '***' SummedByProdTrack, SUM(QTYSTU_0) SumQtyStuByTracking, VCRNUMORI_0, VCRNUM_0
	FROM ECCPROD2.STOJOU
	WHERE VCRNUMORI_0 = @WorkOrder AND TRSTYP_0 IN(5,6,35)
	GROUP BY VCRNUM_0, VCRNUMORI_0
	HAVING SUM(QTYSTU_0) <> 0
	ORDER BY VCRNUMORI_0 DESC
END
...
FROM ECCPROD2.WIPCOST w
WHERE 
	w.TXNTYP_0 = 14
	AND w.VCRNUM_0 IN (SELECT DISTINCT MFGNUM_0 FROM #Trk)
	--
GROUP BY VCRNUM_0
HAVING SUM(w.QTY_0) <> (SELECT CPLQTY_0 FROM ECCPROD2.MFGHEAD h WHERE h.MFGNUM_0 = w.VCRNUM_0)
...
IF @WorkOrder IS NOT NULL
BEGIN
	SELECT 'Bad STOJOU records for missing trackings', * 
	FROM #sto
...
IF @WorkOrder  IS NOT NULL
BEGIN
	SELECT 'STOJOU Problems - Detail' Stojou_Details, s.QTYSTU_0, s.VCRNUMORI_0, s.VCRNUM_0, s.TRSTYP_0, lm.LANMES_0
	FROM ECCPROD2.STOJOU s
		INNER JOIN ECCPROD2.APLSTD lm
			ON s.TRSTYP_0 = lm.LANNUM_0
			AND lm.LANCHP_0 = 704
